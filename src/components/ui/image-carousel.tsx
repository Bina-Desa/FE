...
export default function CarouselWrapper({ images, height = '64' }: { images: string[]; height?: string }) {
        ...
        <CarouselContent>
          {images.map((image, index) => (
            <CarouselItem key={index}>
              <div className="relative">
                <img src={import.meta.env.VITE_BASE_URL + image} className={`rounded-md min-h-80 ${height ? '' : 'max-h-80'} max-h-${height} w-full h-${height} object-cover`} />
